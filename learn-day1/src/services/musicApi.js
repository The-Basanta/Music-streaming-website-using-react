...
export function mapAppleTrack(track) {
  return {
    ...
    cover:
      ...
      PLACEHOLDER_COVER,
    duration: formatMs(track.trackTimeMillis || 210000),
...
async function fetchChartIds(country, limit = 50) {
  try {
    ...
    return (json?.feed?.results || [])
      .map((result) => extractAppleId(result.url))
      .filter(Boolean);
  } catch (error) {
    console.error("Chart fetch error", country, error);
    return [];
  }
}
  ...
  try {
    ...
    (json.results || []).forEach((result) => {
      if (result.wrapperType === "track" && result.kind === "song")
        byId[String(result.trackId)] = result;
    });
    return ids
      .map((id) => byId[id])
      .filter(Boolean)
...
export async function fetchCountryChart(country, limit = 50) {
  return lookupTracks(await fetchChartIds(country, limit));
}
    ...
    fallbackCatalogPromise = Promise.all(
      FALLBACK_SEEDS.map(async (seed) => {
        ...
        const exact = matches.find(
          (track) =>
            track.title.toLowerCase() === seed.title.toLowerCase() &&
            track.artist.toLowerCase().includes(seed.artist.toLowerCase()),
        );
        return exact
          ? { ...exact, genre: seed.genre, country: seed.country }
          : seed;
      }),
    );
  }
  ...
  try {
    ...
    const result = match
      ? {
          id: match.id,
          uri: match.uri,
          duration: formatMs(match.duration_ms),
          durationSeconds: match.duration_ms / 1000,
          cover: match.album.images?.[0]?.url || track.cover,
        }
      : null;
...
export async function searchYouTubeVideoId(query) {
  ...
  try {
    const response = await fetch(
      `https://www.googleapis.com/youtube/v3/search?part=snippet&maxResults=1&type=video&videoCategoryId=10&q=${encodeURIComponent(query)}&key=${YOUTUBE_API_KEY}`,
    );
    const data = await response.json();
